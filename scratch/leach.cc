...
int runLeachSimulation(const struct sensor network[]) 
{
	...
	struct sensor * network_LEACH = NULL;      // wireless sensor network to run sim on
	...
	while (averageEnergy(network_LEACH) > .10) 
	{
		...
		for (i = 0; i < NUM_NODES; i++)
		{
			if (network_LEACH[i].round < (round - recent_round))// || (round - recent_round == 0))
			{
		...
		for (i = 0; i <NUM_NODES; i++) 
		{
			...
			{
				network_LEACH[i].bCurrent -= computeEnergyTransmit(LEACH_AD_DISTANCE, LEACH_AD_MESSAGE);
			}
			else 
			{
				network_LEACH[i].bCurrent -= computeEnergyReceive(LEACH_AD_MESSAGE);
			}
		}
		...
		for (i = 0; i <NUM_NODES; i++) 
		{
			if (network_LEACH[i].head == -1) 
			{
				...
				network_LEACH[i].bCurrent -= computeEnergyTransmit(SCHEDULE_DISTANCE, SCHEDULE_MESSAGE);
			}
			else
				network_LEACH[i].bCurrent -= computeEnergyReceive(SCHEDULE_MESSAGE);
		}
		...
		//check residual energy of cluster head. If residual energy of CH is > threshold, then CH will remain same for next round, else change it... next head with higher energy than the threshold will be cluster head with the same round.(calculate next head)
		// if all the next heads repeats the same round, no node with the energy> threshold, the reclustering phase is initiated(.. incrementing the round count!)
...
int runDirectSimulationNPP(const struct sensor network[]) 
{
	...
	for (j = 0; j <= TOTAL_ROUNDS; j++) 
	{
		for (i = 0; i < NUM_NODES; i++) 
		{
			// cycle through all nodes in network and attempt to   
			// transmit  
			if ((network_DIRECT_NPP[i].bCurrent > 0) && (sensorTransmissionChoice(network_DIRECT_NPP[i]) == 1)) 
			{
				nodes_transmitting++;
			...
			if (network_DIRECT_NPP[i].bCurrent > 0) 
			{
				network_DIRECT_NPP[i].pAverage = ((network_DIRECT_NPP[i].bPower - network_DIRECT_NPP[i].bCurrent) / network_DIRECT_NPP[i].lPeriods);
			}
		}
		if (nodes_transmitting == 0)
			range0_count++;
